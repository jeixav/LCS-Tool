...
u = @(t,x)-p.c3 - p.u*(tanh(x(2))^2 - 1) + 2*p.u*sech(x(2))^2 ...
    *tanh(x(2)) ...
    *(real(p.epsilon1*f1(t)*exp(k(1)*x(1)*lengthscaleX*1i) ...
    + p.epsilon2*f2(t)*exp(k(2)*x(1)*lengthscaleX*1i)) ...
    + p.epsilon3*cos(k(3)*x(1)*lengthscaleX));
v = @(t,x)-p.u*p.lengthY*sech(x(2))^2 ...
    *(imag(p.epsilon1*f1(t)*k(1)*exp(k(1)*x(1)*lengthscaleX*1i) ...
    + p.epsilon2*f2(t)*k(2)*exp(k(2)*x(1)*lengthscaleX*1i)) ...
    + p.epsilon3*k(3)*sin(k(3)*x(1)*lengthscaleX));

bickleyJet.flow.derivative = @(t,x)[2*u(timescale*t,x)/lengthscaleX; ...
    v(timescale*t,x)/lengthscaleY]*timescale;

dux = @(t,x)-2*p.u*sech(x(2))^2*tanh(x(2)) ...
    *(imag(p.epsilon1*f1(t)*k(1)*exp(k(1)*lengthscaleX*x(1)*1i)) ...
    + imag(p.epsilon2*f2(t)*k(2)*exp(k(2)*lengthscaleX*x(1)*1i)) ...
    + p.epsilon3*k(3)*sin(k(3)*lengthscaleX*x(1)));
duy = @(t,x)-2*p.u/p.lengthY*sech(x(2))^2*((3*tanh(x(2))^2 - 1) ...
    *(real(p.epsilon1*f1(t)*exp(k(1)*lengthscaleX*x(1)*1i)) ...
    + real(p.epsilon2*f2(t)*exp(k(2)*lengthscaleX*x(1)*1i)) ...
    + p.epsilon3*cos(k(3)*lengthscaleX*x(1))) + tanh(x(2)));
dvx = @(t,x)-p.u*p.lengthY*sech(x(2))^2 ...
    *(real(p.epsilon1*f1(t)*k(1)^2*exp(k(1)*lengthscaleX*x(1)*1i)) ...
    + real(p.epsilon2*f2(t)*k(2)^2*exp(k(2)*lengthscaleX*x(1)*1i)) ...
    + p.epsilon3*k(3)^2*cos(k(3)*lengthscaleX*x(1)));
dvy = @(t,x)2*p.u*sech(x(2))^2*tanh(x(2)) ...
    *(imag(p.epsilon1*f1(t)*k(1)*exp(k(1)*lengthscaleX*x(1)*1i)) ...
    + imag(p.epsilon2*f2(t)*k(2)*exp(k(2)*lengthscaleX*x(1)*1i)) ...
    + p.epsilon3*k(3)*sin(k(3)*lengthscaleX*x(1)));

bickleyJet.flow.dDerivative = @(t,x)[2*dux(timescale*t,x), ...
    2*duy(timescale*t,x)/lengthscaleX*lengthscaleY; ...
    dvx(timescale*t,x)/lengthscaleY*lengthscaleX, ...
    dvy(timescale*t,x)]*timescale;
